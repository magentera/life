...
attempt("questions.json generation", () => {
  const output = Object.entries(readQuestions()).map(([name, contents]) => {
    const image = getFirstSection(contents)
    const question = getSection("#### Question", contents)
    ...
    const author = getSection("#### Author", contents)
    const date = getSection("#### Date", contents)

    var name = name.slice(0, name.length - 3)
